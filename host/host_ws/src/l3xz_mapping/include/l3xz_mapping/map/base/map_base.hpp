...
{
       public:
        explicit MapBase(std::string name, int8_t coeff_block, int8_t coeff_unblock, int cells_x, int cells_y, double resolution, double preview = 10.0, std::vector<std::shared_ptr<MapPostprocessing>> postprocessing = std::vector<std::shared_ptr<MapPostprocessing>>(), int queue_size = 10); virtual ~MapBase();
