...
template <class T>
MapBase<T>::MapBase(std::string name, int8_t coeff_block, int8_t coeff_unblock, int cells_x,
                    ...
                    int queue_size)
    : MapInterface(name, coeff_block, coeff_unblock, cells_x, cells_y, resolution, preview,
                   postprocessing, tf_parent, tf_child, output_frame),
...
template <class T> void MapBase<T>::add(const T &msg)
{
...
template <class T> void MapBase<T>::worker()
{
    static Timer timer;
    while (_running)
    {
        ...
        if (!_messages.empty())
        {
            ...
            if (_debug)
            {
                ROS_WARN("%s EVAL, %03fs, queue: %i", _name.c_str(), timer.getSeconds(),
                         _messages.size());
            }
            timer.reset();
            postprocess();
