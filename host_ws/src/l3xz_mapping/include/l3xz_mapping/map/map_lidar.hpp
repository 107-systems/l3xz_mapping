...
{
  public:
    explicit MapLidar(std::string name, int8_t coeff_block, int8_t coeff_unblock, double max_dist,
                      double clamp_deg, int cells_x, int cells_y, double resolution,
                      double preview = 10.0,
        ...
        : MapBase<sensor_msgs::LaserScan>(name, coeff_block, coeff_unblock, cells_x, cells_y,
                                          resolution, preview, postprocessing, tf_parent, tf_child,
                                          output_frame, queue_size),
